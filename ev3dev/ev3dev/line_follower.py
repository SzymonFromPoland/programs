...
# while True:
#     print(line_sensor.reflected_light_intensity)
#     sleep(0.1)


sound.speak("Press enter to continue. Down to cance.", play_type=sound.PLAY_WAIT_FOR_COMPLETE)

while True:
    if btn.enter:
        sound.speak("Place on WHITE and press enter.", play_type=sound.PLAY_WAIT_FOR_COMPLETE)
        while True:
            if btn.enter:
                color_upper = line_sensor.reflected_light_intensity
                break
        sound.speak("Place on BLACK and press enter.", play_type=sound.PLAY_WAIT_FOR_COMPLETE)
        while True:
            if btn.enter:
...
sound.speak("Press enter to start.", play_type=sound.PLAY_WAIT_FOR_COMPLETE)

bruh = ((color_upper - color_lower)/2) + 10 # bruh = 75
